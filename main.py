...
GREEN_EMOJIS = {
    ...
    'link': '🔗',
    'crown': '👑',
    'leaf': '🌿',  # Зеленый лист как символ
    'seedling': '🌱',  # Росток
    'herb': '🌿',
    'frog': '🐸',  # Зеленая лягушка
    'monkey': '🐒'
}
...
def get_main_keyboard():
    ...
    builder.adjust(2, 2)  # Расположение кнопок: 2 в первом ряду, 2 во втором
...
async def cmd_start(message: Message):
    ...
    if len(args) > 1:
        ...
        result = c.fetchone()
        if result and result[0] != user_id:  # Не даем пригласить самого себя
...
async def show_profile(message: Message):
    ...
    referral_link = f"https://t.me/{bot_username}?start={user[6]}"  # referral_code
...
async def process_product_data(message: Message, state: FSMContext):
    ...
    await message.answer(f"{GREEN_EMOJIS['success']} Добавлено: {added} позиций\n❌ Ошибок: {errors}")
    await state.clear()
