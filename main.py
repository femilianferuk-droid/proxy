...
async def deliver_product(user_id, product_id, amount_rub):
    ...
    if item:
        ...
        try:
            ...
            builder.row(InlineKeyboardButton(text="📖 Инструкция по настройке", url=instruction_url))
...
async def cmd_start(message: Message):
        ...
        if referred_by:
            try:
                await bot.send_message(
                    ...
                    f"Вы получите 20% от его первых покупок!",
                    parse_mode=ParseMode.HTML
                )
            except Exception as e:
    ...
    welcome_text = (
        ...
        f"🔹 Реферальная система: {REFERRAL_PERCENT}% от покупок друзей\n"
...
async def view_proxy_product(callback: CallbackQuery):
    ...
    if product[8]:  # instruction_url
        builder.row(InlineKeyboardButton(text="📖 Инструкция", url=product[8]))
...
async def show_profile(message: Message):
    ...
    referral_link = f"https://t.me/{bot_username}?start={user[5]}"  # referral_code
    
    profile_text = f"👤 <b>Профиль пользователя</b>\n\n"
    profile_text += f"🆔 ID: <code>{user_id}</code>\n"
    profile_text += f"📅 Дата регистрации: {user[3]}\n"
    profile_text += f"💰 Баланс: {user[4]:.2f}₽ (можно вывести у админа)\n\n"
    
    profile_text += f"🤝 <b>Реферальная система</b>\n"
    profile_text += f"• Ваша ссылка: <code>{referral_link}</code>\n"
    profile_text += f"• Приглашено друзей: {referrals_count}\n"
    profile_text += f"• Заработано всего: {total_earned:.2f}₽\n"
    profile_text += f"• Бонус: {REFERRAL_PERCENT}% от покупок друзей\n\n"
    
    if recent_referrals:
        profile_text += f"📊 <b>Последние рефералы:</b>\n"
    ...
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text="📋 Скопировать реферальную ссылку", callback_data="copy_referral"))
...
async def get_free_key(callback: CallbackQuery):
    ...
    if key:
        ...
        builder = InlineKeyboardBuilder()
        builder.row(InlineKeyboardButton(text="📖 Инструкция", url=instruction_url))
...
async def edit_instruction(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        "Введите новую инструкцию (текст) и ссылку на инструкцию через запятую\n"
...
async def cmd_help(message: Message):
    help_text = (
        ...
        "🔹 <b>Профиль</b> - история покупок, реферальная ссылка и баланс\n"
    ...
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text="📖 Инструкция по прокси", url=f"{INSTRUCTION_SITE}/#proxy"))
    builder.row(InlineKeyboardButton(text="📖 Инструкция по VPN", url=f"{INSTRUCTION_SITE}/#vpn"))
